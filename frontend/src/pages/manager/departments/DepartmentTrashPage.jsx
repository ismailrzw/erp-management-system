...
export const DepartmentTrashPage = () => {
  ...
  const fetchDeleted = useCallback(async (isRefresh = false) => {
    ...
    try {
      const res = await departmentsApi.list({ deleted: true, limit: 100 });
      if (res.success && res.data) {
        setDeletedDepts(res.data.items || res.data || []);
      }
    } catch (err) {
      setToast({
  ...
  const handleRestore = async (dept) => {
    try {
      ...
      await departmentsApi.restore(dept.id || dept._id);
      setToast({ message: `Restored ${dept.name} successfully!`, type: 'success' });
      fetchDeleted(true);
    } catch (err) {
      setToast({
        message: err.response?.data?.message || 'Failed to restore department',
        type: 'error',
      });
    } finally {
  ...
  return (
    <div>
      ...
      >
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13.5px' }}>
            ...
            <tbody>
              {deletedDepts.length === 0 ? (
                <tr>
                  <td colSpan={3} style={{ padding: '36px 20px', textAlign: 'center', color: '#94a3b8' }}>
                    ...
                    <div style={{ fontSize: '12.5px', marginTop: '4px' }}>No deleted departments found.</div>
                  </td>
                </tr>
              ) : (
                deletedDepts.map((d) => (
                  <tr key={d.id || d._id || d.code} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    <td style={{ padding: '12px 16px', color: '#dc2626', fontWeight: 600 }}>{d.code}</td>
                    <td style={{ padding: '12px 16px', color: '#1e293b', fontWeight: 500 }}>{d.name}</td>
      ...
      <Modal isOpen={!!deptToPermanentDelete} onClose={() => setDeptToPermanentDelete(null)} title="Permanent Deletion" maxWidth="440px">
        <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', marginBottom: '20px' }}>
          ...
          <div style={{ fontSize: '13.5px', color: '#475569', lineHeight: 1.5 }}>
            Are you sure you want to permanently delete department <strong>{deptToPermanentDelete?.name}</strong>?
          </div>
        </div>
