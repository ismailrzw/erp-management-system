...
- **Status:** ✅ **Backend Complete & 100% Tested:** All student & manager group endpoints implemented, verified clean with Ruff linter, automated pytest suite passing, and full Swagger OpenAPI documentation.
...
├── app/
│   ├── models/
│   │   ├── group.py                   # Group/Invitation constants (Field, Status, InvitationField, InvitationStatus, REJECTED)
...
│   │   ├── group_service.py           # Pure business logic: group formation, invitations, peer search, browsing
...
│   │   │   ├── groups.py              # Full groups, invitations, browsing, and search namespaces
...
### 3. Student Project Groups (`/api/student/groups`)
| Method | Endpoint | Access | Description |
| :--- | :--- | :---: | :--- |
| `GET` | `/api/student/groups/my` | Student | Get caller's active group with member details (`id`, `name`, `roll`, `section`, `is_leader`) or `null`. |
| `GET` | `/api/student/groups/` | Student | Browse and search active course groups with status filter (`search`, `status`). Orders own group first. |
...
| `POST` | `/api/student/groups/<id>/invite` | Leader Only | Invite a peer by roll number within the same enrolled course (cross-section allowed). |
...
### 7. Read-Only Announcements & Attachments
| Method | Endpoint | Access | Description |
| :--- | :--- | :---: | :--- |
